...
const navItems = [
  ...
  { label: 'Messages', icon: '💬', path: '/dashboard/messages', badge: 1, match: (p) => p.startsWith('/dashboard/messages') },
  ...
  { label: 'My Public Profile', icon: '👤', path: '/dashboard/profile', match: (p) => p.startsWith('/dashboard/profile') },
];
        ...
        <nav className="space-y-1">
          {navItems.map(item => (
            <Link key={item.path} to={item.path}
              ...
              {item.badge && (
                <span className="bg-blue-600 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{item.badge}</span>
              )}
            </Link>
          ))}
        </nav>
